  ...
  <section class="workspace-view workspace-view--todo" aria-label="增强待办">
    ...
    <div class="todo-list">
      <article v-for="item in todos" :key="item.id" class="todo-card">
        ...
        <span class="todo-card__badge" :class="item.level === '紧急' ? 'is-urgent' : 'is-normal'">{{ item.level }}</span>
...
import { DIGITAL_HUMAN_DEVELOPMENT_NOTICE, DIGITAL_HUMAN_TODO_FILTERS, DIGITAL_HUMAN_TODOS } from '@/config/demo-config'

const filters = DIGITAL_HUMAN_TODO_FILTERS
const activeFilter = ref(filters[0])
const todos = DIGITAL_HUMAN_TODOS
